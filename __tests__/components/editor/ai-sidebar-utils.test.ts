...
// In-memory sessionStorage mock (vitest runs in node environment)
...
describe('getSpecFilename', () => {
  ...
  it('falls back to spec.md when the path ends with a slash', () => {
    // split('/').pop() returns '' for a trailing slash — an empty string, not null/undefined.
    // '||' catches empty strings (falsy), so the fallback 'spec.md' is correctly returned.
    expect(getSpecFilename('specs/project-1/')).toBe('spec.md')
  })
  ...
  it('falls back to spec.md for an empty input', () => {
    // ''.split('/').pop() returns '' (not null/undefined). '||' catches the
    // empty string (falsy) and correctly returns the 'spec.md' fallback.
